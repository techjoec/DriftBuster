...
- **Registry keys (all hosts)**, under the host list, names registry keys every host reads (`HKLM\SOFTWARE\Vendor`, optionally `,view=32` or `,view=64`), or an application name whose keys are found from the host's installed applications. Each key is compared like a file: it appears in Compare and Files as `registry/HKLM/SOFTWARE/Vendor.reg`, one setting per value (`[HKEY_LOCAL_MACHINE\SOFTWARE\Vendor\Sub] Port`), with File details showing the key as a Registry Editor export and every right-click action available.
- A host's **Registry** section adds keys for that host only and sets the **Computer** they are read from: blank reads this machine; a name reads that computer over WinRM with Windows PowerShell 5.1 (nothing is installed there). **Sign-in** is the logged-in user unless **Save sign-in…** stores a user name and password as a file only your Windows account on this computer can read (DPAPI, under `credentials/` in the data root), or **Use a saved file…** picks one; **Use my sign-in** goes back to the logged-in user. A host with registry keys needs no file roots. Registry reads need Windows.
- **Remember session** keeps hosts, roots, registry keys and view state in `sessions/multi-server.json` under the data root; **Save**, **Add host** and **Clear** sit beside it.
