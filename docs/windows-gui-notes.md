...
### Reporting adapters shell plan (A11.1.4)

- **Phase 1 – wire CLI payloads**: keep the Python CLI as the primary emitter, invoking it from the GUI through the existing backend bridge. Capture JSON lines to disk, then hand the parsed detections to view-models so the GUI mirrors CLI table output without duplicating parsing logic.
- **Phase 2 – embed HTML summaries**: host `report.html` style snapshots inside a WebView2 panel. Load the file from a temporary evidence directory (`%PROGRAMDATA%/DriftBuster/Reports/`) so auditors can keep the HTML bundle alongside JSON payloads. When the WebView initialises, inject a lightweight toolbar for "open in browser" and "copy evidence path" actions.
- **Phase 3 – diff focus view**: reuse unified diff patches generated by `driftbuster diff` and streamed into the backend. Surface a split viewer in Avalonia where the rendered HTML summary and raw patch sit side by side, letting reviewers pivot between `[REDACTED]`-aware context and the raw diff when permissions allow.
- **Phase 4 – offline audit mode**: add a "Validate evidence" command that replays the CLI scan, recomputes SHA256 checksums for the HTML/JSON/diff bundle, and highlights mismatches before allowing exports. Evidence manifests should align with the `artifacts/reporting/` naming scheme to keep parity with captured smoke runs.
