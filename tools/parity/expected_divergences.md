...
- `LiveHivesTests.OfflineRunnerUsesExplicitRoots` (tests/registry/test_live_hives.py) and `SqlSnapshotsTests.OfflineRunnerSqlSnapshotSource` /
  ...
  written manifest's `sources` entry (and `metadata.sql_exports`) and on the file under the staging data directory. The backend has no
  offline runner (a run profile refuses `registry_scan` and `sql_snapshot` sources); the mirrors run `RegistryScanCollector.Collect` and
  `SqlSnapshotCollector.Collect`, the ports of those two `execute_config` branches, and make the same assertions on the summary, metadata
  entry and file each returns. `scripts/DriftBusterOfflineRunner.Tests.ps1` runs the same tests against the PowerShell offline runner's
  `execute_config` (see "Offline runner script").
...
|---|---|---|
| reading `profile.json` | `OfflineRunnerProfile.from_dict`: non-blank `name`, non-empty `sources`, a mapping through `OfflineCollectionSource.from_dict` (non-empty `path`, blank or falsy `alias` dropped, `bool(optional)`, `exclude` a str or each item's `str()`), any other entry as the path `str(entry)`, a non-null `baseline` that is one of the paths, `tags` iterated, `options` a mapping, a truthy `secret_scanner` a mapping, `description` `str()`; the values are then held as `run_profiles` holds them | `OfflineRunnerProfile.from_dict` over the whole config (see "Offline runner script") |
| `alias` | the source's directory under the run is `_safe_name(alias)` | `_safe_name(alias)` |
| no `alias` | `source_NN`, as `execute_profile` names it (Python: the expanded path's name, else `source_NN`) | Python's: the expanded path's name, else `source_NN` |
| expansion and matching | `os.path.expanduser(os.path.expandvars(path))`, not made absolute; when the path as written holds no `*?[]`, the expanded path if it exists; otherwise the distinct `glob.glob(expanded, recursive=True)` results; nothing found raises `FileNotFoundError("Path does not exist: {path}")`, unless the source is optional (skipped, `ProfileRunResult.Sources` reason `missing` or `no-matches`) | as `offline_runner.py`, relative paths joined under the config file's directory |
| options | `build_context` gets the options as the payload holds them (`RunProfile.SecretOptions`), so a list's items are ignore values and a number or bool is none; `profile.json` holds their `str()` text, as `run_profiles.RunProfile.to_dict()` writes it | `build_context` over the options as the collector config holds them |
| source order | the sources are collected in declared order; the baseline is not moved to the front | same |
| collection | matches in posix order; a match that is a symlink is skipped; a match inside a directory already collected for the source is skipped; a directory's `rglob("*")` files copied in posix order under their path relative to it, a file under its name. Plan decision: nothing inside the run's own output is collected, whether a match names it or lies above it (physical paths compared): the profile directory (`<profiles root>/<profile directory>`, holding `profile.json` and `raw/`) and each directory above it the run created (the Profiles root, or the base directory, when it did not exist before the run). The offline runner writes its output elsewhere, so it never meets that tree: a match that is the run's own output is not a match, and a source whose every match is (`Prof*` when the run created `Profiles/`, `Profiles/ow*/raw` for the profile `own`) raises the `FileNotFoundError` of a source that matches nothing, or is skipped when optional (`closeout-profiles-r2/structured-required-glob-matches-only-run-created-profiles-root`, `structured-required-glob-matches-only-own-profile-dir`; `RunProfileCollectionLimitsTests`) | as `offline_runner.py`; the run-own-output decision is a run profile rule and does not apply |
...
| `exclude` | a pattern matching the file's relative path (posix form) or its name with `fnmatch.fnmatch` skips it | `fnmatch.fnmatch` (case-insensitive on Windows, where it lower-cases both sides) |
...
optional, exclude}`, which that runner reads through `offline_runner`'s own readers.
...
Plan decision (phase 8): the Python entry points are `driftbuster` commands over the backend, one file per command under
`cli/DriftBuster.Cli/Commands/`. Option spellings are Python's, so a documented invocation carries over with only the command word changed.
The parity target of each command is stdout, the exit code and every file written; the domain error texts Python prints (`error: ...`,
`SystemExit` messages, `parser.error` messages) are reproduced.

| Python entry point | Command |
|---|---|
| `python -m driftbuster.cli PATH` | `driftbuster scan PATH` |
| `python -m driftbuster.cli diff` | `driftbuster diff` |
| `python -m driftbuster.cli export-sql`, `driftbuster-export-sql` | `driftbuster sql-export` |
| `python -m driftbuster.run_profiles_cli` (and `cli run-profile`) `create/list/show/run` | `driftbuster profile create/list/show/run` |
| `python -m driftbuster.run_profiles_cli schedule list/due/mark-complete/skip-until` | `driftbuster schedule list/due/mark-complete/skip-until` |
| `python -m driftbuster.profile_cli` | `driftbuster detection-profile` |
| `python -m driftbuster.registry_cli` | `driftbuster registry-scan` |
| `python -m driftbuster.multi_server` (request on stdin) | `driftbuster multi-server` |
| `scripts/capture.py run/export-sql/compare` | `driftbuster capture run/export-sql/compare` |
| `hunt_path(..., return_json=True)` (no command) | `driftbuster hunt` |
| none | `driftbuster report` |

| Case | Python behaviour | Port behaviour |
|---|---|---|
| Argument parsing | `argparse`: its usage line, help text and error wording; option abbreviations (`--out` for `--output`); exit code 2 | System.CommandLine's help and parse errors (plan decision D2): each error as `driftbuster: error: <message>` on stderr with a `--help` hint, exit code 2. No abbreviations; `--option:value` and `-?` are also accepted; response files are off, so `@name` is a value as in argparse. `type=int` / `type=float` convert with `int()` / `float()` (any size) and a refusal reads `argument --x: invalid int value: 'v'`; the `run` group's refusals read as argparse's. A command without its required subcommand exits 2 on both sides |
| `parser.error(message)` raised by command code (`scan` on a missing path, `diff` path and `--context-lines` checks) | usage line, then `<prog>: error: <message>`, exit code 2 | `<prog>: error: <message>` without the usage line, exit code 2 |
| An exception the command does not handle | traceback on stderr, exit code 1 | the traceback's last line, `<Python type name>: <message>` (`CommandRunner.ErrorName`), exit code 1. `SystemExit(message)` writes the message and exits 1 on both sides |
| `run_profiles_cli.main` with no handler | prints help, returns 1 | unreachable: a missing subcommand is a parse error (exit 2) |
| `registry-scan` off Windows | `SystemExit("Registry scanning requires Windows.")` before argv is parsed | the same message and exit code 1 once argv parses; argv that does not parse exits 2 first |
| `--base-dir` of `profile` and `schedule` | top-level option of `driftbuster-run`, before the subcommand | accepted before or after the subcommand |
| stdout and stderr encoding | UTF-8 on Linux; on Windows the console's UTF-8, or the ANSI code page when redirected outside UTF-8 mode | UTF-8 everywhere. LF becomes the platform's line break on both sides (`TextModeWriter`), and files are written with `write_text`'s line breaks |
| stdin encoding (`multi-server` request) | `sys.stdin` decodes with UTF-8 on Linux; on Windows with the console's UTF-8, or the ANSI code page when redirected outside UTF-8 mode | UTF-8 everywhere, a byte order mark skipped (`Program.OpenStandardInput`); not observable on the Linux harness |
| `detection-profile` store that is not valid JSON | `error: Failed to parse JSON from <path>: <JSONDecodeError text>` | `error: Failed to parse JSON from <path>: invalid JSON document`, the "Detection profiles" `_load_json` row |
| `multi-server` with an empty or non-list `plans` | `catalog` and `drilldown` are `{}` | empty arrays, the "Multi-server surface" empty `plans` row |
| `multi-server` request that is not JSON | `Invalid JSON payload: <JSONDecodeError text>` | `Invalid JSON payload: invalid JSON document` (`PythonJson` reports no reason, as the "Scheduling" row records). The response carries fixes a, b and d (see "Multi-server surface"); progress lines follow the runner's throttle |
| `scan --sample-size` past the detector's `int` parameter | the detector clamps with its guardrail warning | the same warning and clamp, applied before the detector is built (as `capture run` does) |
| `diff --content-type auto` | xml when either file's suffix is on `_XML_SUFFIXES` | detection chooses (fix f, `ContentTypeResolver.ResolvePair`) |
| `sql-export` | `cli.py _run_export_sql` | `CaptureRunner.RunSqlExport` with `SqlExportOptions.ManifestName`, `LimitMustBePositive` and `ReportManifestPath`, the three differences from `capture.py`'s export |

Commands without a Python oracle: `hunt PATH [--glob] [--sample-size] [--exclude ...] [--placeholder-template]` prints
`json.dumps(hunt_path(PATH, rules=default_rules(), ..., return_json=True), ensure_ascii=False, sort_keys=True)` and a new line, the serialisation the
JSON lines adapter gives each record, and the array `detection-profile hunt-bridge` reads; a file the hunt could not read (fix b) is named on stderr
as `warning: unreadable file: <path>`. `report [ROOT] [--format html|jsonl] [--glob] [--skip-hunt] [--title] [--mask-token ...] [--placeholder]
[--output]` renders `IDriftbusterBackend.BuildReportAsync` to stdout, or writes `--output` and prints `Report written to <path>`.

Harness handling (`run_parity.sh cli`, `tools/parity/cli_parity.py`): each case under `tools/parity/cases/cli/` runs its Python entry point
(`python -m driftbuster.cli`, `run_profiles_cli`, `profile_cli`, `registry_cli`, `multi_server`, `scripts/capture.py`, or a
`tools/parity/cli_oracles.py` command) and the matching `driftbuster` command in the same freshly copied working directory and environment, and
compares the exit code, stdout bytes, stderr bytes (when the case sets `compare_stderr`) and every file created, changed or removed there. A
case names the acceptors it needs; each is covered by `cli_parity.py --self-test`, and one a case declares but that changes neither side fails
the case:

| Acceptor | Difference it accepts | Rule |
|---|---|---|
| `parse-error` | Argument parsing (D2, first row above) | Both sides exit 2 with empty stdout; Python's stderr holds an argparse `usage:` block followed by `<prog>: error:`, the port's is `driftbuster: error:` lines and the `--help` hint; both stderr texts become one token |
| `usage-line` | `parser.error` from command code (second row) | Python's usage block directly before the `<prog>: error: <message>` line is removed when it exits 2; the message itself is compared |
| `traceback` | Unhandled exception (third row) | Python's `Traceback (most recent call last):` block is cut to its last line when it exits 1; that line is compared |
| `json-decode-reason` | `multi-server` request that is not JSON | Python's error record whose message is `Invalid JSON payload: <reason>: line L column C (char N)` becomes the port's `Invalid JSON payload: invalid JSON document` record, when both exit 1 |
| `json-load-reason` | `detection-profile` store that is not valid JSON | Python's `error: Failed to parse JSON from <path>: <reason>: line L column C (char N)` stderr line becomes the port's `...: invalid JSON document` line when the port printed exactly that line and both exit 1 |
| `empty-response-mappings` | `multi-server` with an empty or non-list `plans` | Python's `"results": [], "catalog": {}, "drilldown": {}` becomes `"results": [], "catalog": [], "drilldown": []` when the port's stdout carries the latter and both exit 0 |
| `run-clock` | Wall-clock readings (SQL manifest `captured_at`, HTML `Generated at`, multi-server and capture timestamps, capture ids and run profile directories named after the clock) | An ISO 8601 UTC timestamp or `%Y%m%dT%H%M%SZ` id inside that side's run window (2 s slack) is replaced by its shape (digits as `#`) in stdout, stderr, file bytes and file paths; the format is still compared |
| `capture-durations` | `capture run` manifest `durations` | The three measured values, non-negative numbers with at most three decimal places, become `0` in `*-manifest.json` |
| `oracle-fixes` | Fixes a, b and d (`multi-server`), e (`hunt`, `report`) and f (`diff`) | The Python side is a `cli_oracles.py` command applying the fix as the multi-server, hunt and diff surfaces apply it (`_MultiServerFixes`, `_PortXmlCanonical`, `_hunt_rules(fix_e=True)`, `_resolve_pair`) and is compared exactly; a third run without the fixes (`PARITY_CLI_STOCK=1`) must differ, and counts as one expected divergence |

`registry-scan` cases run off Windows compare the refusal; on Windows the same cases compare the command output. `capture run`
cases run the stock script, so their trees hold no Windows install path (fix e inside `capture run` is compared on the capture surface).

Test mirror notes: `cli/DriftBuster.Cli.Tests` runs the argv halves in process through `Program.Run`. `CliTests` mirrors tests/cli/test_cli.py:
`MainReturnsCodeWhenParserErrorIsSuppressed` calls `ScanCommand.Execute`, which returns 2 itself, where Python stubs `parser.error`;
`MainConsumesSysArgv`, `ExportSqlConsoleMainForwardsArguments` and `CliDiffModuleEntrypointSupportsMultipleComparisons` run the same argv through
the root command instead of `sys.argv`, the `driftbuster-export-sql` shim and a subprocess. `RunProfilesCliCommandsTests` mirrors
tests/cli/test_run_profiles_cli_commands.py with argv in place of hand-built namespaces; `MainReturnsErrorWhenNoCommand` asserts a non-zero exit
with the error on stderr (the row above), and `RunProfilesModuleMain` that `profile list` parses to the `list` command. `ProfileCliTests`
(tests/cli/test_profile_cli.py) swaps `DetectionProfileCommands.FromDict` where Python monkeypatches `ProfileStore.from_dict`, and
`ParseArgsRequiresCommand` expects exit code 2 where Python expects `SystemExit`. `RegistryCliTests` (tests/registry/test_registry_cli.py) and
`ScheduleIntegrationTests` (tests/scheduler/test_integration.py) keep Python's inputs and assertions.

## Offline runner script (`scripts/driftbuster-offline-runner.ps1`)

The offline runner is one Windows PowerShell 5.1 script with nothing to install and nothing beside it (the offline collector package ships
it alone): the PowerShell port of `offline_runner.execute_config` (config readers, file and glob sources, `registry_scan` over
`Microsoft.Win32.RegistryKey`, with `RegQueryValueExW` for the value types `RegistryKey.GetValue` does not read, `sql_snapshot` over
`winsqlite3.dll`, DPAPI/AES package encryption) and C# helpers inside the script that `Add-Type` compiles at load with the .NET Framework
compiler (Python value semantics, `json`, Python's UTF-8 codec with one U+FFFD per maximal ill-formed subpart, which the .NET Framework
decoder does not give, `os.path`/`pathlib`/`glob`/`fnmatch`, `copy_with_secret_filter` with fix g, the SQLite P/Invoke wrapper, which loads
`libsqlite3.so.0` on Linux so PowerShell 7 can test it).
`scripts/DriftBusterOfflineRunner.Tests.ps1` dot-sources the script for its helpers and ports the Python tests (the registry and DPAPI
tests tagged `Windows` run on a Windows host). The `offline-run` surface (`run_parity.sh offline-run` for the Linux cases,
a Windows host for the Windows cases, both through `tools/parity/offline_runner_parity.py`) runs
`execute_config_path` and the script over each case under `tools/parity/cases/offline-run/` in the same run directory and compares the
result or error text, every entry left in the run directory and the package's entries after the acceptors that tool documents (run clock,
rewritten-content digests, decrypted and MAC-verified encrypted packages, and `host.platform` off Windows).

| Case | Python behaviour | Runner behaviour | Handling |
|---|---|---|---|
| Secret rules, ignore patterns and registry patterns | CPython `re` | .NET `Regex` (culture-invariant; flag `i` is `IgnoreCase`). `\s` does not match U+001C-U+001F; case-insensitive matching does not fold U+017F or U+212A as `re` does; `.` matches one UTF-16 unit; `\Z` also matches before a final new line; Python-only syntax such as `(?P<name>...)` does not compile, so such a rule is dropped as Python drops a rule `re` refuses; a registry pattern that does not compile raises `PatternError` with .NET's message | No case; the `offline-run` cases use rules both engines read alike, and the packaged ruleset matches identically on `fixtures/secret_samples` and the secret cases |
| `fnmatch` and `glob` patterns over names outside the BMP | `?` and a character class match one code point | They match one UTF-16 unit | No case |
| Character tables: `_safe_name`'s `isalnum`, `str.strip`, `lower`/`upper`, `repr`'s printable set | Python 3.13 Unicode tables and special casing | The runtime's categories (.NET Framework 4.8 under Windows PowerShell); `ToLowerInvariant`/`ToUpperInvariant` without special casing | No case |
| JSON nested deeper than 1000 containers (config, keyset, inline ruleset) | Decoded up to the interpreter's limit | `RecursionError: maximum recursion depth exceeded while decoding a JSON array (object) from a unicode string` | Interpreter limit; no case |
| An integer literal of more than 4300 digits in a config | `ValueError` from the integer conversion limit | Parsed | Interpreter limit; no case |
| `JSONDecodeError` position after a character outside the BMP | Counted in code points | Counted in UTF-16 units | Error text; no case |
| `host.platform` in the manifest | `platform.platform()` | On Windows the same composition (`Windows-<release>-<version>-SP<n>` from the WMI operating system record and `platform.py`'s release tables); elsewhere `Environment.OSVersion.VersionString` | Compared on Windows; the `host-platform` acceptor off Windows (counted as an expected divergence) |
| The zip package | `zipfile` with `ZIP_DEFLATED` | `System.IO.Compression.ZipArchive`: the same entry names, order and contents; the compressed bytes, sizes and header fields differ, so an encrypted package's `size` and `sha256` differ | The harness compares entries; an encrypted package's `size` is checked against its plaintext and its digest follows the rewritten envelope |
| `-ConfigPath` / `-OutputDirectory` given relative to the script | `execute_config_path` keeps a relative config path as given | The script resolves both against the PowerShell location, so the manifest's `config.path` is absolute. `Invoke-DbOfflineRunnerPath` keeps a relative path as Python does | Script entry point only |
| A database SQLite can only open writable (a hot journal to roll back, a WAL database without a writable `-shm`) | `sqlite3.connect` opens read-write and recovers it | Opened read-only (plan decision 7), so the open fails. SQL error texts come from the platform's SQLite (`winsqlite3.dll`) where Python's come from the SQLite it bundles | No case |
| A DPAPI blob whose decryption needs a prompt | `CryptUnprotectData` without flags | `ProtectedData.Unprotect` passes `CRYPTPROTECT_UI_FORBIDDEN`, so it fails with the same `RuntimeError` instead of prompting | No case |
| A registry scan that reaches `time_budget_s` | Stops after the keys CPython walked in the budget | PowerShell walks fewer keys per second, so it may stop with fewer hits | Timing; the cases use budgets they finish within |
| An output the runner cannot create or write (a `package_name` naming a missing directory, a `log_name` Windows refuses) | `OSError` with its errno text | The .NET exception | Error text; no case |
| `~user` off Windows; `..` after a symlink off Windows | `pwd.getpwnam`; the kernel resolves `..` | `/etc/passwd`; existence and type checks use `statx` on the path, but .NET's other file calls remove `..` lexically | Linux is a test platform only |
