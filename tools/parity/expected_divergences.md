...
`str()`, `schedule` the `JSONDecodeError` text and the call budget at the nesting limit, and `multi-server` adds fixes a, b and d, the undecodable-name platform limit, the unpaired-surrogate raises and the `diff_summary` port-model limit. `detect` without `--plugins` runs the full default registry on both sides
