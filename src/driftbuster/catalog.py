...
FORMAT_SURVEY = FormatSurvey(
    ...
    formats=(
        ...
        FormatUsage(
            ...
            extensions=(".ini", ".cfg", ".cnf"),
            mime_hints=("text/plain",),
            context="Classic INI-style config with [Section] headers and key=value pairs.",
            approx_usage_percent=15,
            confidence_model="section-header+eq-sign",
        ),
        FormatUsage(
            format="properties",
            variant="key-value-properties",
            extensions=(".properties",),
            mime_hints=("text/plain",),
            context="Property files using key=value or key:value pairs across runtime configuration systems.",
            approx_usage_percent=3,
            confidence_model="eq-sign+colon",
        ),
        FormatUsage(
            format="unix-conf",
            variant=None,
            extensions=(".conf",),
            mime_hints=("text/plain",),
            context="Unix-style directive configuration for server and database stacks on desktop platforms.",
            approx_usage_percent=2,
            confidence_model="comment-semicolon-or-hash",
        ),
        FormatUsage(
        ...
        FormatUsage(
            ...
            variant="engine-hybrid",
            extensions=(".ini",),
            mime_hints=("text/plain",),
            context="Hybrid INI/JSON configuration style used by game engines requiring structured metadata.",
            approx_usage_percent=0.5,
            confidence_model="section-header+curly-braces",
        ),
        FormatUsage(
            format="env-file",
            variant="dotenv",
            extensions=(".env",),
            mime_hints=("text/plain",),
            context="Environment variable definition files for cross-platform apps and Docker Compose.",
            approx_usage_percent=1,
            confidence_model="key-eq-value-lines",
        ),
    ),
    meta=SurveyMeta(
        primary_key="format",
