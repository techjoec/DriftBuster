...
The session captured no raw payloads. `DiffViewModel` writes its structured log to `artifacts/logs/diff-planner-telemetry.json`
when caching and replaying sanitized entries.
