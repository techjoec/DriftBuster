...
/// </summary>
internal static class PythonArguments
{
    ...
    private static BigInteger? ParseInt(ArgumentResult result, string name)
    {
        ...
        try
        {
            return PythonBuiltins.Int(text);
        }
        catch (PythonValueException)
        {
            result.AddError($"argument {name}: invalid int value: {PythonRepr.StrRepr(text)}");
            return null;
        }
    }
    ...
    private static double? ParseFloat(ArgumentResult result, string name)
    {
        ...
        try
        {
            return PythonBuiltins.Float(text);
        }
        catch (PythonValueException)
        {
            result.AddError($"argument {name}: invalid float value: {PythonRepr.StrRepr(text)}");
            return null;
        }
    }
}
