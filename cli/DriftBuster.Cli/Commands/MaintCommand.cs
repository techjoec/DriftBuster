...
internal static class MaintCommand
{
    ...
    {
        var portableRoot = PythonArguments.Text(
            "--portable-root",
        ...
        var output = PythonArguments.OptionalText("--output", "Path to write JSON report (default: artifacts/selfcheck/multi_server_paths_report.json).");
    ...
    private static Command BuildPurge()
    {
        ...
        var retentionDays = PythonArguments.Int("--retention-days", 30, "Retention window in days (default: 30)");
        var confirm = PythonArguments.Flag("--confirm", "Actually delete candidates; otherwise prints a dry-run report");
