...
/// <summary>
/// The Windows calls <c>ntpath.realpath</c> makes (<see cref="PythonNtRealPath"/>), so the algorithm runs over a fake on every host.
...
internal interface INtPathSystem
{
    ...
    /// <exception cref="PythonValueException">The path holds a NUL character (<c>embedded null character</c>).</exception>
    ...
    /// <exception cref="PythonValueException">The reparse point is not a link (<c>not a symbolic link</c>).</exception>
