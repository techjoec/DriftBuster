...
/// <summary><c>datetime.fromisoformat</c>, ported from CPython 3.13's C parser.</summary>
public sealed partial class PythonDateTime
{
    ...
    /// </summary>
    public static PythonDateTime FromIsoFormat(string text)
    {
    ...
    private static PythonValueException InvalidIsoFormat(string text)
        => new("Invalid isoformat string: " + PythonRepr.StrRepr(text), nameof(text));

    // tzinfo_from_isoformat_results: UTC for a zero second offset (whatever the microseconds), otherwise timezone(timedelta(...)).
    private static PythonFixedOffset OffsetZone(IsoFields fields)
        => fields.TzOffset == 0
            ? PythonFixedOffset.Utc
            : PythonFixedOffset.Create(PythonTimeDelta.FromMicroseconds(((long)fields.TzOffset * 1_000_000) + fields.TzMicrosecond));
