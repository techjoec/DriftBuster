...
/// </summary>
/// <remarks>
/// <c>PythonReOracleTests</c> compares the category, decimal and digit values, <c>str.isalpha()</c>, <c>str.isalnum()</c> and
/// <c>str.isprintable()</c> with the interpreter's over every code point (<c>tools/parity/gen_regex_cases.py</c> writes them); when
/// the runtime's tables change, the category test fails with the delta table to paste here. Numeric values (<c>str.isnumeric()</c>)
/// are not covered: the runtime lacks the Unihan numeric values of some ideographs, and nothing ported reads them.
/// </remarks>
public static class PythonUnicode
{
