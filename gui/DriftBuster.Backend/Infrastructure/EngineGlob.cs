...
public static class EngineGlob
{
    ...
    public static IReadOnlyList<string> Glob(string root, string pattern, CancellationToken cancellationToken = default)
    {
        ...
        var rootText = EnginePurePath.Str(root);
        ...
        foreach (var path in globber.Select(0, rootText, exists: false))
        {
            ...
            results.Add(EnginePurePath.Str(normalised));
        }
    ...
    {
        var pureParts = EnginePurePath.Parts(pattern);
        if (EnginePurePath.Anchor(pattern).Length > 0)
        {
            throw new EngineNotImplementedException("Non-relative patterns are unsupported");
        }

        var parts = pureParts.ToList();
        if (parts.Count == 0)
        {
            var repr = (Windows ? "WindowsPath(" : "PosixPath(") + EngineRepr.StrRepr(EnginePurePath.Str(pattern)) + ")";
            throw new EngineValueException("Unacceptable pattern: " + repr, nameof(pattern));
        }
    ...
    private static int AnchorLength(string path) => EnginePurePath.Anchor(path).Length;
    ...
    private sealed class Globber(List<string> parts, CancellationToken cancellationToken)
    {
        ...
        {
            if (exists || File.Exists(EnginePath.KernelPath(path)) || Directory.Exists(EnginePath.KernelPath(path)))
            {
                yield return path;
            }
        }
        ...
        private IEnumerable<string> SelectWildcard(int index, string path)
        {
            ...
            var match = string.Equals(part, "*", StringComparison.Ordinal)
                ? null
                : EnginePattern.Compile(EnginePurePath.GlobTranslate(part, Separator.ToString()), Windows ? EngineReFlags.IgnoreCase : EngineReFlags.None);
        ...
        private List<string> ScanDirectory(string path)
        {
            ...
            try
            {
                return [.. Directory.EnumerateFileSystemEntries(path.Length == 0 ? "." : EnginePath.KernelPath(path)).Select(Path.GetFileName).OfType<string>()];
            }
    ...
    private static bool IsDirectory(string path, bool followSymlinks)
    {
        try
        {
            ...
            var attributes = File.GetAttributes(EnginePath.KernelPath(path));
    ...
    private static string AddSlash(string path)
    {
        if (Windows)
        {
            var tail = EngineNtPath.SplitRoot(path).Remainder;
