...
public sealed class EngineFixedOffset : EngineTzInfo
{
    private const long MicrosecondsPerSecond = 1_000_000;

    private EngineFixedOffset(EngineTimeDelta offset)
    {
        Offset = offset;
    }

    /// <summary><c>datetime.UTC</c>.</summary>
    public static EngineFixedOffset Utc { get; } = new(default);

    public EngineTimeDelta Offset { get; }
    ...
    public static EngineFixedOffset Create(EngineTimeDelta offset)
    {
        ...
        return new EngineFixedOffset(offset);
    }

    public override long UtcOffset(EngineDateTime moment) => (long)Offset.TotalMicroseconds;
