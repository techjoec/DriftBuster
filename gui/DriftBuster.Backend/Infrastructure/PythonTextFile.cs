...
/// </remarks>
internal static class PythonTextFile
{
    ...
    /// <c>open()</c>'s error for one (<see cref="PythonOSError.DirectoryOpenErrno"/>) and any other failure Python's <c>OSError</c> text
    /// (<see cref="PythonOSError"/>), the path spelled as <c>str(Path)</c> spells it when <c>open()</c> raised, and no path when the write did.
    /// </summary>
    /// <exception cref="PythonValueException">The path holds a NUL character (<c>open()</c>'s <c>embedded null byte</c>).</exception>
    ...
    public static void WriteBytes(string path, byte[] bytes, string? shown = null)
    {
        ...
        PythonOSError.ThrowIfEmbeddedNull(path);
        shown ??= PythonPurePath.Str(path);
        if (RunProfileStore.IsDirectory(path))
        {
            throw PythonOSError.Create(PythonOSError.DirectoryOpenErrno, shown);
        }
    ...
    /// (<see cref="PythonOSError.DirectoryOpenErrno"/>), a failed open Python's <c>OSError</c> text naming the path and a failed read the
    /// text without it; bytes that are not UTF-8 raise <see cref="PythonUnicodeDecodeException"/>.
    /// </summary>
    /// <exception cref="PythonValueException">The path holds a NUL character (<c>open()</c>'s <c>embedded null byte</c>).</exception>
    public static string ReadUtf8Text(string path) => PythonUtf8.Decode(ReadBytes(path, path));
    ...
    private static Stream Open(string path, string shown, FileMode mode)
    {
        try
        {
            return OpenStream(PythonPath.KernelPath(path), mode);
        }
        catch (Exception exc) when (exc is IOException or UnauthorizedAccessException)
        {
            throw OpenError(exc, path, shown);
    ...
    private static Exception OpenError(Exception exc, string path, string shown)
        => exc.Message.StartsWith("[Errno ", StringComparison.Ordinal) || PythonOSError.Errno(exc, path) is not { } known
            ? exc
            : PythonOSError.Create(known, shown, exc);
    ...
    private static Exception AfterOpenError(Exception exc)
        => exc is ArgumentOutOfRangeException
            ? PythonOSError.Create(PythonOSError.FileTooLarge, exc)
            : PythonOSError.Errno(exc) is { } known ? PythonOSError.Create(known, exc) : exc;
}
