...
{
    public static ReCharSet? Of(ReSubPattern pattern, PythonReFlags flags)
    {
        var (set, nullable) = Sequence(pattern, flags);
        return nullable || set.SetEquals(CodePointSet.All) ? null : new ReCharSet(set);
    }

    private static (CodePointSet Set, bool Nullable) Sequence(ReSubPattern pattern, PythonReFlags flags)
    {
    ...
    private static (CodePointSet Set, bool Nullable) Node(ReNode node, PythonReFlags flags)
    {
        switch (node)
        {
