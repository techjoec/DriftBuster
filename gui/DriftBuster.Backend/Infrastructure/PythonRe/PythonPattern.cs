...
/// matching engine, ported.
/// </summary>
/// <remarks>
/// <para>
/// <see cref="Compile"/> raises <see cref="PythonReException"/> wherever <c>re.compile</c> raises <c>re.error</c>,
...
/// Offsets on <see cref="PythonMatch"/> are UTF-16 indices into the subject string, ready for slicing; a code point offset
...
/// </para>
/// </remarks>
public sealed class PythonPattern
{
    private const int CacheLimit = 512;

    private static readonly ConcurrentDictionary<(string Pattern, PythonReFlags Flags), PythonPattern> Cache = new();

    private readonly ReProgram _program;

    private PythonPattern(string pattern, ReSubPattern parsed, PythonReFlags flags)
    {
        Pattern = pattern;
    ...
    public PythonReFlags Flags { get; }
    ...
    public static PythonPattern Compile(string pattern, PythonReFlags flags = PythonReFlags.None)
    {
        ...
        var compiled = new PythonPattern(pattern, ReParser.Parse(pattern, flags), flags);
    ...
    public PythonMatch? Match(string text, CancellationToken cancellationToken = default)
    {
    ...
    public PythonMatch? Search(string text, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(text);
        var subject = new Subject(text);
        var matcher = NewMatcher(subject, cancellationToken);
    ...
    /// </summary>
    public IEnumerable<PythonMatch> FindIter(string text, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(text);
        return FindIterCore(text, cancellationToken);
    }

    private IEnumerable<PythonMatch> FindIterCore(string text, CancellationToken cancellationToken)
    {
        var subject = new Subject(text);
        var matcher = NewMatcher(subject, cancellationToken);
        var start = 0;
    ...
    private sealed class Subject
    {
        ...
        public PythonMatch MatchFrom(PythonPattern pattern, ReMatcher matcher)
        {
            ...
            return new PythonMatch(pattern, Text, spans, matcher.LastIndex < 0 ? null : matcher.LastIndex);
        }
