...
/// <summary>
/// A port of CPython 3.13's <c>re._compiler._code</c> / <c>_compile</c> (PSF License): the same opcode layout, skip offsets and
...
/// prefix and charset only speed up <c>_sre</c>'s search; the port keeps its minimum width and an equivalent first-set filter
...
internal sealed class ReCompiler
{
    ...
    public static ReProgram Compile(ReSubPattern pattern, PythonReFlags flags)
    {
    ...
    private void CompileSequence(ReSubPattern pattern, PythonReFlags flags)
    {
    ...
    private void CompileNode(ReNode node, PythonReFlags flags)
    {
        switch (node)
        {
    ...
    private void CompileRepeat(ReNode.Repeat repeat, PythonReFlags flags)
    {
    ...
    private void CompileGroup(ReNode.Subpattern group, PythonReFlags flags)
    {
    ...
    private void CompileAssert(ReNode.Assert assertion, PythonReFlags flags)
    {
        ...
        else
        {
            ...
            if (low > MaxCode)
            {
                throw new PythonReException("looks too much behind", position: null);
            }

            if (low != high)
            {
                throw new PythonReException("look-behind requires fixed-width pattern", position: null);
            }
    ...
    private void CompileBranch(ReNode.Branch branch, PythonReFlags flags)
    {
    ...
    private void CompileReference(ReNode node, PythonReFlags flags)
    {
        if (node is ReNode.GroupRef reference)
        {
            Emit(!flags.HasFlag(PythonReFlags.IgnoreCase) ? ReOpcode.GroupRef
                : flags.HasFlag(PythonReFlags.Unicode) ? ReOpcode.GroupRefUniIgnore : ReOpcode.GroupRefIgnore);
    ...
    private static ReAtCode AtCodeFor(AtCode code, PythonReFlags flags)
    {
        var multiline = flags.HasFlag(PythonReFlags.Multiline);
        var unicode = flags.HasFlag(PythonReFlags.Unicode);
        return code switch
        {
    ...
    internal static PythonReFlags CombineFlags(PythonReFlags flags, PythonReFlags add, PythonReFlags del)
    {
        const PythonReFlags typeFlags = PythonReFlags.Ascii | PythonReFlags.Locale | PythonReFlags.Unicode;
        if ((add & typeFlags) != PythonReFlags.None)
        {
            flags &= ~typeFlags;
        }
