...
/// or <c>dir_fd</c> and <c>include_hidden=False</c>. It differs from <c>Path.glob</c> (<see cref="EngineGlob"/>): the pattern
...
/// the runtime would reach the entry its U+FFFD spelling names. Python looks a <c>surrogateescape</c> spelling
...
/// </remarks>
public static class EngineModuleGlob
{
    ...
    {
        var (dirname, basename) = EngineOsPath.Split(pathname);
        ...
        foreach (var directory in dirs)
        {
            ...
            foreach (var name in names)
            {
                yield return EngineOsPath.Join(directory, name);
            }
        }
    }
    ...
    private static List<string> Glob1(string dirname, string pattern, bool dirOnly, CancellationToken cancellationToken)
        => ListDir(dirname, dirOnly, cancellationToken)
            ...
            .Where(name => EngineFnmatch.Fnmatch(name, pattern))
            .ToList();
    ...
    private static string JoinParts(string dirname, string basename)
        => dirname.Length == 0 || basename.Length == 0 ? dirname + basename : EngineOsPath.Join(dirname, basename);
    ...
    private static List<string> ListDir(string dirname, bool dirOnly, CancellationToken cancellationToken)
    {
        ...
        if (EngineUtf8.HasUnpairedSurrogate(directory))
        {
            return [];
        }
        ...
        try
        {
            var names = ListedNames(directory, Directory.EnumerateFileSystemEntries(EnginePath.KernelPath(directory)).Select(Path.GetFileName).OfType<string>());
            return dirOnly ? names.Where(name => IsDir(EngineOsPath.Join(directory, name))).ToList() : names;
        }
    ...
    private static List<string> ListedNames(string directory, IEnumerable<string> names)
    {
        ...
        return names.Where(name => !name.Contains('\uFFFD', StringComparison.Ordinal) || (replaced.Add(name) && LExists(EngineOsPath.Join(directory, name)))).ToList();
    }
