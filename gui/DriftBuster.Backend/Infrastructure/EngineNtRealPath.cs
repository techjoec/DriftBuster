...
/// </summary>
internal static class EngineNtRealPath
{
    ...
    public static string RealPath(string path, INtPathSystem system)
    {
        ...
        path = EngineNtPath.NormPath(path);
        ...
        if (!hadPrefix && !EngineNtPath.IsAbs(path))
        {
            path = EngineNtPath.Join(system.CurrentDirectory, path);
        }
    ...
    private static string GetFinalPathNameNonStrict(string path, INtPathSystem system)
    {
        ...
        while (path.Length > 0)
        {
            ...
            {
                path = system.GetFinalPathName(path);
                return tail.Length > 0 ? EngineNtPath.Join(path, tail) : path;
            }
            catch (NtPathException exc)
            {
            ...
            try
            {
                ...
                {
                    return tail.Length > 0 ? EngineNtPath.Join(newPath, tail) : newPath;
                }
            }
            ...
            if (FindName.Contains(failure.WinError))
            {
                try
                {
                    ...
                    path = EngineNtPath.Split(path).Head;
                }
                catch (NtPathException)
                {
                    (path, name) = EngineNtPath.Split(path);
                }
            }
            else
            {
            ...
            tail = tail.Length > 0 ? EngineNtPath.Join(name, tail) : name;
        }
        ...
        while (seen.Add(NormCase(path)))
        {
            ...
            try
            {
                ...
                if (!EngineNtPath.IsAbs(path))
                {
                    ...
                    path = EngineNtPath.NormPath(EngineNtPath.Join(EngineNtPath.Split(oldPath).Head, path));
                }
            }
            catch (NtPathException exc)
            {
    ...
    private static string NormCase(string path) => EngineText.Lower(path.Replace('/', '\\'));
}
