...
internal static partial class ReParser
{
    private static void ParseConditional(ReTokenizer source, ReParseState state, ReSubPattern subpattern, bool verbose, int nested, int start)
    {
        ...
        if (!condName.All(ch => ch is >= '0' and <= '9'))
        {
            ...
            {
                throw source.Error($"unknown group name {EngineRepr.StrRepr(condName)}", nameLength + 1);
            }
        }
        else
        {
    ...
    private static (EngineReFlags Add, EngineReFlags Del)? ParseFlags(ReTokenizer source, ReParseState state, int code)
    {
        var add = EngineReFlags.None;
        var del = EngineReFlags.None;
        int[]? token = null;
        if (code != '-')
        {
            while (true)
            {
                ...
                add |= flag;
                if ((flag & TypeFlags) != EngineReFlags.None && (add & TypeFlags) != flag)
                {
    ...
    private static EngineReFlags ParseDeletedFlags(ReTokenizer source)
    {
        var del = EngineReFlags.None;
        ...
        while (true)
        {
            ...
            if ((flag & TypeFlags) != EngineReFlags.None)
            {
    ...
    private static bool IsAlpha(int[] token)
        => token.All(EngineUnicode.IsAlpha);
}
