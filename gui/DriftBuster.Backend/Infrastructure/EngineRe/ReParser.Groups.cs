...
{
    private const EngineReFlags TypeFlags = EngineReFlags.Ascii | EngineReFlags.Locale | EngineReFlags.Unicode;

    private static EngineReFlags? FlagFor(int code) => code switch
    {
        'i' => EngineReFlags.IgnoreCase,
        'L' => EngineReFlags.Locale,
        'm' => EngineReFlags.Multiline,
        's' => EngineReFlags.DotAll,
        'x' => EngineReFlags.Verbose,
        'a' => EngineReFlags.Ascii,
        'u' => EngineReFlags.Unicode,
        _ => null,
    };
    ...
    private static bool ParseParen(ReTokenizer source, ReParseState state, ReSubPattern subpattern, bool verbose, int nested, bool first)
    {
        ...
        {
            case 'P':
                return ParseNamedGroupExtension(source, state, subpattern, verbose, nested, start);
            case ':':
        ...
        if (code == '-' || FlagFor(code) is not null)
        {
            ...
            if (flags is null)
            {
                ...
                return state.Flags.HasFlag(EngineReFlags.Verbose);
            }
    ...
    private static bool ParseNamedGroupExtension(ReTokenizer source, ReParseState state, ReSubPattern subpattern, bool verbose, int nested, int start)
    {
        if (source.Match('<'))
        {
        ...
        if (source.Match('='))
        {
            ...
            {
                throw source.Error($"unknown group name {EngineRepr.StrRepr(name)}", nameLength + 1);
            }
    ...
    private readonly record struct GroupSpec(bool Capture, bool Atomic, string? Name, EngineReFlags Add = EngineReFlags.None, EngineReFlags Del = EngineReFlags.None);
    ...
    private static void AddGroup(ReTokenizer source, ReParseState state, ReSubPattern subpattern, GroupSpec spec, bool verbose, int nested, int start)
    {
        ...
        if (spec.Capture)
        {
            ...
            catch (EngineReException error)
            {
                throw source.Error(error.Message, ReTokenizer.CodePoints(spec.Name ?? string.Empty).Length + 1);
            }
        }

        var subVerbose = (verbose || spec.Add.HasFlag(EngineReFlags.Verbose)) && !spec.Del.HasFlag(EngineReFlags.Verbose);
