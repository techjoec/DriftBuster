...
/// to "S"). <c>EngineReCaseTests</c> checks the lowercase table and the cased set against the interpreter over every
/// code point, and the category sets through the atom cases. No code point at or above U+20000 changes case, so the change
/// tables stop there.
/// </remarks>
internal static class EngineCharacterData
{
    ...
    private static readonly Lazy<CodePointSet> UnicodeSpaceSet = new(() => CodePointSet.FromPredicate(code => code <= 0xFFFF && !IsSurrogate(code) && EngineText.IsSpace((char)code)));
    ...
    private static Dictionary<int, int> BuildChanges(Func<Rune, int> map)
    {
        ...
        {
            if (IsSurrogate(code) || !MayChangeCase(EngineUnicode.GetCategory(code)))
            {
                continue;
            }
    ...
    // skipping every other category keeps the one-time scan cheap. The regex case tests compare the result over every code point.
    ...
    private static int UpperCore(Rune rune) => Rune.GetRuneAt(EngineText.Upper(rune), 0).Value;

    // Py_UNICODE_ISALNUM or '_': isalpha (L*), isdecimal, isdigit and isnumeric, which over Unicode's categories are the
    // letters and the three number categories (EngineText.IsWordRune).
    private static bool IsUnicodeWord(int code) => !IsSurrogate(code) && EngineText.IsWordRune(new Rune(code));

    // Py_UNICODE_ISDECIMAL: category Nd.
    private static bool IsUnicodeDigit(int code) => EngineUnicode.IsDecimal(code);
}
