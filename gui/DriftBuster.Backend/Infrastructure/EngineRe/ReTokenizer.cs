...
internal sealed class ReTokenizer
{
    ...
    public EngineReException Error(string message, int offset = 0) => new(message, Tell() - offset);

    public void CheckGroupName(string name, int offset)
    {
        if (!EngineIdentifier.IsIdentifier(name))
        {
            throw Error($"bad character in group name {EngineRepr.StrRepr(name)}", CodePoints(name).Length + offset);
        }
    }
    ...
    private void Advance()
    {
        ...
        if (code == '\\')
        {
            ...
            if (index >= _codes.Length)
            {
                throw new EngineReException("bad escape (end of pattern)", _codes.Length - 1);
            }
