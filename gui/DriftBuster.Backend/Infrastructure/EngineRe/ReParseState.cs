...
internal sealed class ReParseState
{
    ...
    public EngineReFlags Flags { get; set; }
    ...
    public int OpenGroup(string? name)
    {
        ...
        if (Groups > MaxGroups)
        {
            throw new EngineReException("too many groups");
        }
        ...
        if (name is not null)
        {
            ...
            {
                throw new EngineReException(
                    $"redefinition of group name {EngineRepr.StrRepr(name)} as group {gid}; was group {existing}");
            }
