...
/// <summary>
/// Derived from CPython 3.13's <c>re._parser</c> (PSF License) producing the same syntax tree and raising the same
...
internal static partial class ReParser
{
    ...
    public static ReSubPattern Parse(string pattern, EngineReFlags flags)
    {
        var source = new ReTokenizer(pattern);
        var state = new ReParseState { Flags = flags };
        var parsed = ParseSub(source, state, flags.HasFlag(EngineReFlags.Verbose), 0);
        ...
        foreach (var (group, position) in state.GroupRefPositions)
        {
            ...
            {
                throw new EngineReException($"invalid group reference {group}", position);
            }
        }
    ...
    private static EngineReFlags FixFlags(EngineReFlags flags)
    {
        if (flags.HasFlag(EngineReFlags.Locale))
        {
            throw new EngineValueException("cannot use LOCALE flag with a str pattern", nameof(flags));
        }

        if (!flags.HasFlag(EngineReFlags.Ascii))
        {
            return flags | EngineReFlags.Unicode;
        }

        if (flags.HasFlag(EngineReFlags.Unicode))
        {
            throw new EngineValueException("ASCII and UNICODE flags are incompatible", nameof(flags));
        }
    ...
    private static ReSubPattern ParseSub(ReTokenizer source, ReParseState state, bool verbose, int nested)
    {
        ...
        while (true)
        {
            ...
            if (nested == 0)
            {
                verbose = state.Flags.HasFlag(EngineReFlags.Verbose);
            }
        }
