...
public static partial class PythonOsPath
{
    ...
    private static string ExpandUserPosix(string path)
    {
        ...
        else
        {
            ...
            // the port does not reproduce (the root keeps its surrogate and is never looked up), and a NUL raises ValueError.
            if (PythonUtf8.HasUnpairedSurrogate(name))
            {
                return path;
            }
            ...
            {
                throw new PythonValueException("embedded null byte", nameof(path));
            }
