...
/// <c>PythonZoneInfo.Runtime.cs</c>).
/// </summary>
public sealed partial class PythonZoneInfo : PythonTzInfo
{
    ...
    private static readonly ConcurrentDictionary<string, PythonZoneInfo> Cache = new(StringComparer.Ordinal);

    private readonly TzifZone? _tzif;

    private PythonZoneInfo(string key, TzifZone? tzif, TimeZoneInfo? runtime)
    {
        Key = key;
        _tzif = tzif;
        _runtime = runtime;
    ...
    /// accepts (<see cref="PythonValueException"/> otherwise); on Windows it must be an IANA id the runtime resolves.
    /// </summary>
    public static PythonZoneInfo Create(string key)
    {
        ...
        ValidateKey(key);
        var zone = OperatingSystem.IsWindows() ? new PythonZoneInfo(key, tzif: null, FindRuntimeZone(key)) : new PythonZoneInfo(key, LoadTzif(key), runtime: null);
    ...
    public override long UtcOffset(PythonDateTime moment)
    {
    ...
    private static void ValidateKey(string key)
    {
        ...
        {
            throw new PythonValueException($"ZoneInfo keys may not be absolute paths, got: {key}", nameof(key));
        }

        var normalised = PythonOsPath.NormPath(OperatingSystem.IsWindows() ? key.Replace('/', '\\') : key);
        if (normalised.Length != key.Length)
        {
            throw new PythonValueException($"ZoneInfo keys must be normalized relative paths, got: {key}", nameof(key));
        }

        if (!PythonOsPath.NormPath("_/" + normalised).StartsWith("_/", StringComparison.Ordinal))
        {
            throw new PythonValueException($"ZoneInfo keys must refer to subdirectories of TZPATH, got: {key}", nameof(key));
        }
    }
    ...
    // cannot be read, is not found (the port's key lookup); data zoneinfo refuses raises ValueError (a UnicodeDecodeError is one).
    private static TzifZone LoadTzif(string key)
    {
        ...
        catch (PythonUnicodeDecodeException exc)
        {
            throw new PythonValueException(exc.Message, nameof(key), exc);
        }
    }
    ...
    private static byte[] ReadZoneFile(string key)
    {
        ...
        var path = PythonOsPath.Join(directory, key);
        return PythonPath.IsFile(path) ? File.ReadAllBytes(path) : [];
    }
