...
/// </summary>
/// <remarks>
/// A path is parsed as <c>PurePath._parse_path</c> parses it: the flavour's <c>splitroot</c> (<see cref="EngineNtPath.SplitRoot"/> on
...
/// through <c>glob.translate(part, include_hidden=True, seps=sep)</c> compiled with <see cref="EnginePattern"/> (case-insensitively
/// on Windows); an anchored pattern must cover the whole path. <c>**</c> is an ordinary wildcard here, as it is in
/// <c>PurePath.match</c>.
/// </remarks>
public static class EnginePurePath
{
    ...
    internal static string Format(string drive, string root, IEnumerable<string> tail, bool windows)
    {
        ...
        return windows && names.Count > 0 && EngineNtPath.SplitDrive(names[0]).Drive.Length > 0
    ...
    {
        ArgumentNullException.ThrowIfNull(path);
        return windows ? EngineNtPath.IsAbs(Str(path, windows)) : path.StartsWith('/');
    }
    ...
    /// <summary>
    /// The flavour's <c>os.path.splitroot(path)</c>: <see cref="EngineNtPath.SplitRoot"/> on Windows; on posix no drive, a root of
    ...
    internal static (string Drive, string Root, string Remainder) SplitRoot(string path, bool windows)
    {
        ...
        if (windows)
        {
            return EngineNtPath.SplitRoot(path);
        }
    ...
    internal static string Join(string path, string relative, bool windows)
    {
        ...
        if (windows)
        {
            return Str(EngineNtPath.Join(path, relative), windows);
        }
    ...
    private static string NormCase(string text, bool windows) => windows ? EngineText.Lower(text) : text;
    ...
    internal static bool Match(string path, string pattern, bool windows)
    {
        ...
        {
            throw new EngineValueException("empty pattern", nameof(pattern));
        }
        ...
        var flags = windows ? EngineReFlags.IgnoreCase : EngineReFlags.None;
        var separator = windows ? "\\" : "/";
        for (var index = 0; index < patternParts.Count; index++)
        {
            var compiled = EnginePattern.Compile(GlobTranslate(patternParts[index], separator), flags);
