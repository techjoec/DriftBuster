...
/// <c>fnmatchcase</c> over <see cref="PythonPattern"/> (whose compile cache stands in for <c>_compile_pattern</c>'s), and
...
/// </summary>
public static class PythonFnmatch
{
    ...
    public static bool FnmatchCase(string name, string pattern)
    {
        ArgumentNullException.ThrowIfNull(name);
        ArgumentNullException.ThrowIfNull(pattern);
        return PythonPattern.Compile(Translate(pattern)).Match(name) is not null;
    }
    ...
    public static string Translate(string pattern)
    {
        ArgumentNullException.ThrowIfNull(pattern);
        var star = new string(['.', '*']);
        var parts = PythonPurePath.FnmatchTranslateParts(pattern, star, ".");
