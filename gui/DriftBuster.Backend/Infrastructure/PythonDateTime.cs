...
/// <summary>
/// CPython 3.13's <c>datetime.datetime</c>: year 1 to 9999 fields with microseconds, an optional <see cref="PythonTzInfo"/> and the
...
public sealed partial class PythonDateTime
{
    ...
    private PythonDateTime(DateTime fields, PythonTzInfo? tz, int fold)
    {
    ...
    public static PythonDateTime Create(
        int year,
        int month,
        int day,
        int hour = 0,
        ...
        int second = 0,
        int microsecond = 0,
        PythonTzInfo? tz = null,
        int fold = 0)
    {
        if (year is < 1 or > 9999)
        {
            throw new PythonValueException(string.Create(CultureInfo.InvariantCulture, $"year {year} is out of range"), nameof(year));
        }

        if (month is < 1 or > 12)
        {
            throw new PythonValueException("month must be in 1..12", nameof(month));
        }

        if (day < 1 || day > DateTime.DaysInMonth(year, month))
        {
            throw new PythonValueException("day is out of range for month", nameof(day));
        }

        PythonTime.CheckTimeArgs(hour, minute, second, microsecond);
        if (fold is not (0 or 1))
        {
            throw new PythonValueException("fold must be either 0 or 1", nameof(fold));
        }

        var fields = new DateTime(year, month, day, hour, minute, second).AddTicks(microsecond * TicksPerMicrosecond);
        return new PythonDateTime(fields, tz, fold);
    }

    /// <summary><c>datetime.now(UTC)</c>: the current time to the microsecond on <see cref="PythonFixedOffset.Utc"/>.</summary>
    public static PythonDateTime UtcNow()
    {
        var now = DateTime.UtcNow;
        return new PythonDateTime(new DateTime(now.Ticks - (now.Ticks % TicksPerMicrosecond)), PythonFixedOffset.Utc, 0);
    }
    ...
    public PythonDateTime WithTz(PythonTzInfo? tz) => new(_fields, tz, Fold);

    /// <summary><c>dt.replace(fold=fold)</c>.</summary>
    public PythonDateTime WithFold(int fold) => new(_fields, Tz, fold);

    /// <summary><c>dt.replace(hour=..., minute=..., second=..., microsecond=...)</c>: unspecified fields, the tzinfo and the fold are kept.</summary>
    public PythonDateTime Replace(int? hour = null, int? minute = null, int? second = null, int? microsecond = null)
    ...
    public PythonTime TimeOfDay() => PythonTime.FromFields(Hour, Minute, Second, Microsecond);

    /// <summary><c>dt + delta</c>.</summary>
    public PythonDateTime Add(PythonTimeDelta delta) => FromFieldMicroseconds(FieldMicroseconds + delta.TotalMicroseconds, Tz);

    /// <summary><c>dt + timedelta(microseconds=microseconds)</c>.</summary>
    internal PythonDateTime AddMicroseconds(long microseconds) => FromFieldMicroseconds((BigInteger)FieldMicroseconds + microseconds, Tz);
    ...
    /// </summary>
    public PythonDateTime AsTimeZone(PythonTzInfo tz)
    {
        ArgumentNullException.ThrowIfNull(tz);
        if (Tz is null)
        {
            throw new NotSupportedException("astimezone() of a naive datetime uses the system local time zone, which the port does not model.");
        }
    ...
    public string IsoFormat()
    {
        ...
        return UtcOffset() is { } offset ? text + PythonFixedOffset.FormatOffset(offset) : text;
    }
    ...
    /// </summary>
    /// <exception cref="PythonTypeException">One side is naive and the other aware.</exception>
    public int CompareWith(PythonDateTime other)
    {
        ...
        {
            throw new PythonTypeException("can't compare offset-naive and offset-aware datetimes", nameof(other));
        }
    ...
    public static bool operator <(PythonDateTime left, PythonDateTime right) => Order(left, right) < 0;
    ...
    private static int Order(PythonDateTime left, PythonDateTime right)
    {
        ArgumentNullException.ThrowIfNull(left);
        return left.CompareWith(right);
    }

    private static PythonDateTime FromFieldMicroseconds(BigInteger microseconds, PythonTzInfo? tz)
    {
        ...
        return new PythonDateTime(new DateTime((long)microseconds * TicksPerMicrosecond), tz, 0);
    }
}
