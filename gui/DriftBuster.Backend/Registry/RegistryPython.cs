...
/// <summary>
/// Python built-ins the registry port needs that the shared helpers do not cover: <c>str.upper()</c> over a whole string,
/// <c>re.split</c>, <c>re.compile</c>'s error text and <c>type(exc).__name__</c> for the exceptions the registry code raises.
/// </summary>
internal static class RegistryPython
{
    /// <summary><c>text.upper()</c>: <see cref="PythonText.Upper(Rune)"/> per code point, an unpaired surrogate passed through.</summary>
    public static string Upper(string text)
    {
        ...
        while (offset < text.Length)
        {
            ...
            builder.Append(PythonText.Upper(rune));
            offset += consumed;
        }
    ...
    public static List<string> Split(PythonPattern pattern, string text)
    {
    ...
    /// <summary>
    /// <c>re.compile(pattern)</c>. A pattern that does not compile raises <see cref="PythonReException"/> whose message is
    ...
    /// </summary>
    public static PythonPattern Compile(string pattern)
    {
        try
        {
            return PythonPattern.Compile(pattern);
        }
        catch (PythonReException exc) when (exc.Position is { } position)
        {
            ...
            throw new PythonReException(message, position);
        }
    }
    ...
    /// <summary>
    /// <c>type(exc).__name__</c> of the Python exception each port exception stands for: <c>RuntimeError</c> is
